...
public class Window_Loging extends JFrame implements ActionListener {
	...
	public Window_Loging() {
		...
		Jbut_Acount = new JButton("회원가입");
		Jbut_Acount.addActionListener(this);
		Jbut_Acount.setBounds(74, 134, 89, 27);
		add(Jbut_Acount);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(obj == Jbut_login) {
			...
			if(id.equals("")) {
				...
				dto.setId(id);
				dto.setPw(pw);
				boolean Loging = s.memCtrl.select_loging(dto);
			if(Loging == true) {
				dispose();	
			}else if(Loging == false) {
				JOptionPane.showMessageDialog(null, "아이디 또는 비밀번호가 틀리셨습니다.");
			}
		}
		}else if(obj == Jbut_Acount) {
			dispose();
			s.Win_Acount = new Window_ACount();
