...
public class OrderView extends JFrame implements ActionListener, ItemListener {
	...
	public OrderView() {
		...
		paymentBtn.addActionListener(new ActionListener() { // 결제버튼 눌렀을 때
				...
				if (yesNoBtn == JOptionPane.YES_OPTION) {
					...
					for (int i = 0; i < dtm.getRowCount(); i++) {
						if ((boolean) dtm.getValueAt(i, 0) == true) { // 체크되어 있으면
							...
							System.out.println("confiremdList = " + confirmedList);
						}
					}
	...
	private void setDeliveryTable() {
		...
		OrderData[0][3] = date.format(today); // 현재날짜
